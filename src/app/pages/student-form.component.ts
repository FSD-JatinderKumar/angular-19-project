...
import { AbstractControl, FormBuilder, FormGroup, ReactiveFormsModule, UntypedFormBuilder, Validators } from '@angular/forms';
import { Router, ActivatedRoute, RouterModule } from '@angular/router';
import { Title } from '@angular/platform-browser';
import { provideHttpClient } from '@angular/common/http';


import { countries } from './student-form/countries-list';
// Add any other required modules (e.g., NgbModule, FormsModule)
import { FormsModule } from '@angular/forms';

// Custom services, pipes, components
import { AuthService } from '../../APIServices/auth.service';
import { SemesterExchangeStuDetailsService } from '../../APIServices/semester-exchange-stu-details.service';
...
export class StudentFormComponent implements OnInit {
  ...
  semesterService = inject(SemesterExchangeStuDetailsService);
  ...
  //   const loginNameFromRoute = this.route.snapshot.paramMap.get('loginName');
  //   if (loginNameFromRoute) {
  //     this.LoginName = loginNameFromRoute;
  //     this.getToken(this.LoginName);
  //   } else {
  //     this.LoginFailed('Missing login name in URL.');
  //     return;
  //   }
  ...
  //   if (stMain) {
  //     stMain.innerHTML = 'Semester <span class="text-info">Exchange </span>Registration';
  //   }
  ...
  ngOnInit(): void {
    ...
    const loginNameFromRoute = this.route.snapshot.paramMap.get('loginName');
    // const loginNameFromRoute = this.route.snapshot.paramMap.get('loginName');
    console.log('Route loginName:', loginNameFromRoute); // 👈 Confirm this prints
    alert(0)
    if (loginNameFromRoute) {
      this.LoginName = loginNameFromRoute;
      this.getToken(this.LoginName);
    } else {
      this.LoginFailed('Login name missing in route.');
      return;
    }
  ...
  getToken(id: any) {
    this.AuthServicess.loginTemp(id).subscribe({
      next: data => {
        ...
        if (!authToken || authToken === 'Token Expired' || !this.StorageServicess.isLoggedIn())
